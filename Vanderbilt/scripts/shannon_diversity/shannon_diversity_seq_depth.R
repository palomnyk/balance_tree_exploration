...
# Script for Shannon sequence depth vs metadata
...
for (i in 1:max(result_df$mds_lev)){
  g <- ggplot2::ggplot(melted_results[melted_results$mds_lev == i, ],
    ...
    ggplot2::ggtitle(paste0(project, ", PCA",i," Seq depth vs Rsqd of Shannon Diversity vs metadata")) +
...
g <- ggplot2::ggplot(melted_results,
  ...
  ggplot2::geom_point(aes(color = factor(Metadata))) +
  ggplot2::geom_line(aes(color = factor(Metadata))) +
  ggplot2::ggtitle(paste0(project, ", PCA",i," Read depth vs P-val of Shannon Diversity vs metadata")) +
  ggplot2::xlab("Min sequence depth per sample") +
  ggplot2::ylab("Pvale") +
  ggplot2::labs(fill = "Metadata") +
  theme(axis.text.x = element_text(angle = 90)) +
...
g <- ggplot2::ggplot(result_df,
  ...
  ggplot2::geom_point() +
  ggplot2::geom_line() +
  ggplot2::ggtitle(paste0(project, ": Number samples vs read depth filt")) +
  ggplot2::xlab("Min sequence depth per sample") +
