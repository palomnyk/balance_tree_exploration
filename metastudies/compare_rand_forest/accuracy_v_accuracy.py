...
for ds1 in comp_ds:
	...
	for ds2 in comp_ds:
		if (ds1 != ds2):
			...
			ds1_lst = np.array(list(ds1_score.values()))
			ds2_lst = np.array(list(ds2_score.values()))
			...
			ax.scatter(ds1_score.values(), ds2_score.values(), color = "blue", label=f"{ds1} vs {ds2}")
